...
def main():
    ...
    try:
        if category_kpis_df is not None:
            write_category_kpis_to_dynamodb(category_kpis_df, table_name="category_kpi_table", session=session)
        else:
            logger.warning("Category KPIs DataFrame is None, skipping DynamoDB write.")
        
        if order_kpis_df is not None:
            write_order_kpis_to_dynamodb(order_kpis_df, table_name="order_kpi_table", session=session)
        else:
